...
def updateGrid(frame):
    ...
    ghost_u[0::, 1::len(u)-2] =  2*wall_v - u[0::]
    ghost_u[::-1] = -u[::-1]
    ...
    ghost_v[0::, 1::len(u)-2] =  2*wall_v - v[0::]
    ghost_v[0::, 1::len(u)-2] = -v[::-1]
    ...
    v = old_v + t*(-old_u*(left_v - right_v)/2 - old_v*(up_v - down_v)/2) + (1/re)*(left_v + right_v + up_v + down_v - 4*u) ## vert velocity
