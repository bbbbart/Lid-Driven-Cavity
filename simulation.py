...
## create quiver plot and heatmaps
...
def updateGrid(frame):
    ...
    if not pause and running:
        ...
        for i in range(50):
            ...
            ghost_v[1:-1, 1:-1] = v
            ghost_v[[0, -1], 1:-1] = -v[[0, -1], :] ## stationary wall
            ghost_v[1:-1, [0, -1]] = -v[:, [0, -1]] ## stationary wall

            ghost_u[1:-1, 1:-1] = u
            ghost_u[0, 1:-1] =  -1*(u[0,:]) + 2*var.wall_v ## moving wall
            ghost_u[-1, 1:-1] = -u[-1,:] ## stationary wall
            ghost_u[1:-1, [0, -1]] = -u[:, [0, -1]] ## stationary wall
            ...
            ## calculate initial velocity using first-order upwind
            ...
            ## update boundary conditions and neighbors
            ...
            u = u - var.t*(right_p - left_p)/(2*var.delta_x)
            v = v - var.t*(down_p - up_p)/(2*var.delta_y)
